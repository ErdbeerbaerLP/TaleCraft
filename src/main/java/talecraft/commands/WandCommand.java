...
public class WandCommand extends TCCommandBase {
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length <= 1) {
            return getListOfStringsMatchingLastWord(args, "region",
                    "expand",
                    "expand_h",
                    "expand_v",
                    "expand_to",
        ...
        if (args[0].equals("region")) {
            if (args.length == 2) {
                return getListOfStringsMatchingLastWord(args, "erase", "fill", "replace", "room", "repeat", "copy", "paste", "trigger", "butcher");
            }
