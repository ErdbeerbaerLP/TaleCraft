...
public class VoxelBrushCommand extends CommandBase {
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) {
            return getListOfStringsMatchingLastWord(args, "shape", "action");
        }

        if (args.length >= 1) {
            if (args[0].equals("shape") && args.length <= 2) {
                return getListOfStringsMatchingLastWord(args, "box", "cylinder", "sphere");
            }

            // func_175762_a(args, Block.blockRegistry.getKeys())

            if (args.length == 2 && args[0].equals("action")) {
                return getListOfStringsMatchingLastWord(args, "replace", "maskreplace", "grassify", "erase");
            }
