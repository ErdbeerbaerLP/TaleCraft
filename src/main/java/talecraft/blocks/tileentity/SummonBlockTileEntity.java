...
public class SummonBlockTileEntity extends TCTileEntity {
	...
	public SummonBlockTileEntity() {
		super(TaleCraftRegistered.TE_SUMMON);
	...
	public void readFromNBT_do(NBTTagCompound comp) {
		summonCount = comp.getInt("summonCount");
		summonRegionBounds = comp.getIntArray("summonRegionBounds");
		useWeightAsCount = comp.getBoolean("useWeightAsCount");

		NBTTagList list = comp.getList("summonOptions", NBT.TAG_COMPOUND);
		summonOptions = new SummonOption[list.size()];

		for(int i = 0; i < list.size(); i++) {
			summonOptions[i] = new SummonOption().read(list.getCompound(i));
		}
	}
	...
	public NBTTagCompound writeToNBT_do(NBTTagCompound comp) {
		comp.setInt("summonCount", summonCount);
		...
		for(int i = 0; i < summonOptions.length; i++) {
			...
			if(option != null)
				list.add(option.write());
			else
				TaleCraft.logger.severe(getName() + " : Option #" + i + " is NULL! -> " + list);
		}
	...
	public void trigger(EnumTriggerState on) {
		...
		if(useWeightAsCount) {
			...
			// for(int i = 0; i < summonCount; i++)
			{
				for(SummonOption option : summonOptions) {
					for(int j = 0; j < option.getWeight(); j++) {
						if(!option.isStable() || stableCheck(EntityType.create(option.getData(), world), (int) option.getWeight()))summonEntity(option);
					}
				}
			}
		} else {
			...
			for(int i = 0; i < summonCount; i++) {
				...
				if(!option.isStable() || stableCheck(EntityType.create(option.getData(), world), summonCount))summonEntity(option);
			}
		}
	}
	...
	public void summonEntity(SummonOption option) {
		...
		Entity entity = EntityType.create(entityNBT, world);

		if(entity == null) {
			TaleCraft.logger.severe("FAILED TO SUMMON ENTITY: " + option.getData());
			return;
		}
		...
		// This takes care of 'riding' entities.
		{
			...
			for (
					...
					mountEntity != null && mountEntityNBT.hasKey("Riding");
					mountEntityNBT = mountEntityNBT.getCompound("Riding")
					) {
				Entity ridingEntity = EntityType.create(mountEntityNBT.getCompound("Riding"), world);
	...
	public static class SummonOption {
		...
		public void setData(NBTTagCompound entityData) {
			if(entityData == null) {
				TaleCraft.logger.severe("'entityData' is null!");
			}
		...
		public SummonOption read(NBTTagCompound compound) {
			...
			summonData = compound.getCompound("summonData");

			String ID = summonData.getString("id");

			if(ID != null && !ID.isEmpty() && summonData.keySet().size() < 3) {
				Entity entity = EntityType.create(null, new ResourceLocation(ID));
				if(entity != null) {
					NBTTagCompound mergecompound = new NBTTagCompound();
					entity.writeUnlessRemoved(mergecompound);
