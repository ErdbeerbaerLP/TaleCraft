...
public class SummonBlockTileEntity extends TCTileEntity {
    ...
    public void init() {
        if (Arrays.equals(summonRegionBounds, ZEROBOUNDS)) {
            ...
            summonRegionBounds[1] = this.pos.getY();
    ...
    public void trigger(EnumTriggerState on) {
        ...
        if (useWeightAsCount) {
            ...
            // for(int i = 0; i < summonCount; i++)
            {
                for (SummonOption option : summonOptions) {
                    for (int j = 0; j < option.getWeight(); j++) {
                        if (!option.isStable() || stableCheck(EntityList.createEntityFromNBT(option.getData(), world), (int) option.getWeight()))
                            summonEntity(option);
                    }
                }
            }
            ...
            for (int i = 0; i < summonCount; i++) {
                ...
                if (!option.isStable() || stableCheck(EntityList.createEntityFromNBT(option.getData(), world), summonCount))
                    summonEntity(option);
            }
        }
    }
    ...
    public static class SummonOption {
        ...
        public SummonOption read(NBTTagCompound compound) {
            ...
            if (ID != null && !ID.isEmpty() && summonData.getKeySet().size() < 3) {
                @SuppressWarnings("ConstantConditions") Entity entity = EntityList.createEntityByIDFromName(new ResourceLocation(ID), null);
                if (entity != null) {
