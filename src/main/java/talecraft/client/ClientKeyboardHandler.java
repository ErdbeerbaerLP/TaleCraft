    ...
    public void on_key(KeyInputEvent event) {
        ...
        if (nbt.isPressed() && nbt.isKeyDown() && mc.player != null && mc.player != null && !mc.isGamePaused()) {
            ...
            if (!player.getCurrentItem().isEmpty()) {
                if (player.getCurrentItem().hasTagCompound())
                    FMLCommonHandler.instance().showGuiScreen(new GuiNBTEditor(player.getCurrentItem().getTagCompound()));
                else
                    mc.player.sendMessage(new TextComponentString(TextFormatting.RED + "This item does not have NBT data..."));
            } else
                mc.player.sendMessage(new TextComponentString(TextFormatting.RED + "You must be holding something to use the NBT Editor"));
            return;
        }
