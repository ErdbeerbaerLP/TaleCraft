...
public class QADDropdownBox extends QADRectangularComponent {
	...
	public void handleMouseInput() {
		int amount = Input.getEventDWheel()/120;
		if(amount < 0){
...
//				default:color = 0xFF101010;break;
			...
			if(items != null) {
				...
				// hover highlight
				double hoverlight = hover?mouseToItemIndex(localMouseY+getY()):-1;
...
//				default:color = 0xFF101010;break;
	...
	@Override
	public void onMouseReleased(double localMouseX, double localMouseY, int mouseButton) {
		...
		if(isFocused && inside && dropbox_model != null) {
			List<ListModelItem> items = dropbox_model.getFilteredItems();
			if(items != null) {
				double hoverlight = mouseToItemIndex(localMouseY+getY());
				// this.dropbox_highlight = hoverlight;
				
				if(hoverlight != -1 && hoverlight < items.size()) {
					ListModelItem item = items.get((int)hoverlight);
	...
	@Override
	public void mouseDragged(double d, double e, int p_mouseDragged_5_, double p_mouseDragged_6_,
	...
	public void onKeyTyped(char typedChar, int typedCode) {
		if(isFocused) {
			...
			if(typedCode == GLFW.GLFW_KEY_TAB) {
				// autocompletion?
				// Not necessary; see below.
				return;
			}
			
			if(typedCode == GLFW.GLFW_KEY_ENTER) {
				if(dropbox_model != null) {
					List<ListModelItem> items = dropbox_model.getFilteredItems();
					if(items.size() > 0) {
			...
			if(typedCode == GLFW.GLFW_KEY_BACKSPACE) {
	...
	public boolean isPointInside(double d, double e) {
		boolean inside =
				d >= this.fieldbox_x &&
				e >= this.fieldbox_y &&
				d <= this.fieldbox_x + this.fieldbox_width &&
				e <= (dropbox_visible?(this.dropbox_y+this.dropbox_height):(this.fieldbox_y+this.fieldbox_height));
		return inside;
	}
	
	public double mouseToItemIndex(double d) {
		if(d < dropbox_y) {
			return -1;
		}
		if(d >= dropbox_y+dropbox_height) {
			return -1;
		}
		...
		return (d/dropbox_rowheight) +  + dropbox_scroll;
	}
