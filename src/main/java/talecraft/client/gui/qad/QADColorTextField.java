...
public class QADColorTextField extends QADRectangularComponent {
	...
	public QADColorTextField(int xPos, int yPos, int width, int height) {
		// this.ID = __id__;
		this.model = new DefaultTextFieldModel();
		this.fontRendererInstance = Minecraft.getInstance().fontRenderer;
		this.xPosition = xPos;
		this.yPosition = yPos;
		this.width = width;
		this.height = height;
	...
	public void onKeyTyped(char p_146201_1_, int p_146201_2_) {
		...
		{
			clipboard.setContents(new StringSelection(this.getSelectedText()), new StringSelection(this.getSelectedText()));
			return;
		}
		else if (GuiScreen.isKeyComboCtrlV(p_146201_2_))
		{
			...
			{
				try {
					this.writeText((String) clipboard.getData(DataFlavor.stringFlavor));
		...
		else
		{
			...
			default:
				...
				if (SharedConstants.isAllowedCharacter(p_146201_1_)) 
				{
					if (this.isEnabled)
					{
						this.writeText(Character.toString(p_146201_1_));
...
//		System.out.println("\u3432");
		...
		String s1 = "";
		String s2 = SharedConstants.filterAllowedCharacters(newText);
		s2 = s2.replace("\u3432", "\u00A7");
		int i = this.cursorPosition < this.selectionEnd ? this.cursorPosition : this.selectionEnd;
		int j = this.cursorPosition < this.selectionEnd ? this.selectionEnd : this.cursorPosition;
	...
	public static interface TextChangeListener {
		public void call(QADColorTextField qadTextField, String text);
	}
