...
public class PanelDrops extends NPCPanel {
    ...
    public PanelDrops(NPCData data, int width, int height) {
        ...
        drops = new ArrayList<>(data.getDrops());
    ...
    public QADPanel getPanel() {
        ...
        moveDown.setAction(() -> moveIndex(false));
        ...
        moveUp.setAction(() -> moveIndex(true));
        ...
        if (drops.size() > 0) {
            ...
            @SuppressWarnings("ConstantConditions") QADTextField nbtField = new QADTextField(drops.get(index).stack.getTagCompound().toString());
