...
public class VCUIRenderer {
    ...
    private void do_drawTexturedModalRect(float xCoord, float yCoord, int minU, int minV, int maxU, int maxV) {
        ...
        vertexbuffer.pos(xCoord + 0.0F, yCoord + maxV, this.zLevel).tex((minU) * f2, (minV + maxV) * f3).endVertex();
        vertexbuffer.pos(xCoord + maxU, yCoord + maxV, this.zLevel).tex((minU + maxU) * f2, (minV + maxV) * f3).endVertex();
        vertexbuffer.pos(xCoord + maxU, yCoord + 0.0F, this.zLevel).tex((minU + maxU) * f2, (minV) * f3).endVertex();
        vertexbuffer.pos(xCoord + 0.0F, yCoord + 0.0F, this.zLevel).tex((minU) * f2, (minV) * f3).endVertex();
        tessellator.draw();
    }
    ...
    private void do_drawTexturedModalRect(int xCoord, int yCoord, TextureAtlasSprite textureSprite, int width, int height) {
        ...
        BufferBuilder vertexbuffer = tessellator.getBuffer();
        vertexbuffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        vertexbuffer.pos(xCoord, yCoord + height, this.zLevel).tex(textureSprite.getMinU(), textureSprite.getMaxV()).endVertex();
        vertexbuffer.pos(xCoord + width, yCoord + height, this.zLevel).tex(textureSprite.getMaxU(), textureSprite.getMaxV()).endVertex();
        vertexbuffer.pos(xCoord + width, yCoord, this.zLevel).tex(textureSprite.getMaxU(), textureSprite.getMinV()).endVertex();
        vertexbuffer.pos(xCoord, yCoord, this.zLevel).tex(textureSprite.getMinU(), textureSprite.getMinV()).endVertex();
        tessellator.draw();
    }
    ...
    public void pushScissor(int x, int y, int width, int height) {
        ...
        @SuppressWarnings("ConstantConditions") float factorX = MathHelper.ceil((float) Display.getWidth() / (float) minecraft.currentScreen.width);
