...
public class PushRenderableFactory {
	...
	public static final ITemporaryRenderable parsePushRenderableFromNBT(NBTTagCompound data) {
		...
		if(type.equals("box")) {
			...
			ren.box = box;
			ren.color = data.hasKey("color") ? data.getInt("color") : 0xFFFF7F00;
		...
		if(type.equals("highlight.block")) {
			...
			int posX = data.getInt("pos.x");
			int posY = data.getInt("pos.y");
			int posZ = data.getInt("pos.z");
			BlockPos pos = new BlockPos(posX, posY, posZ);
			return new BlockHighlight(pos, duration);
		}

		TaleCraft.logger.severe("Unknown push renderable: " + type);
		return null;
	}
