package talecraft.render.tileentity;
...
@OnlyIn(Dist.CLIENT)
public class GenericTileEntityRenderer<T extends TileEntity> extends TileEntityRenderer<T> {
	...
	@Override
	public void render(T tile, double posX, double posY, double posZ, float partialTicks, int destroyStage) {
	...
	//*/
		...
		//	if(TaleCraft.isBuildMode() && tile instanceof IInvokeSource && ClientProxy.settings.getBoolean("client.render.invokeVisualize")) {
		//		GlStateManager.pushMatrix();
		//		GlStateManager.translatef(
		//				(float) -this.rendererDispatcher.entityX,
		//				(float) -this.rendererDispatcher.entityY,
