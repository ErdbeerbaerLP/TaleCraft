...
//			TaleCraft.network.sendTo(new PlayerNBTDataMergePacket(entity.getEntityData()), (EntityPlayerMP) entity);
	...
	private static void handleSNBTCommand(EntityPlayerMP player, World world, StringNBTCommandPacket message) {
		if(world.isRemote){
			TaleCraft.logger.severe("FATAL ERROR: ServerHandler method was called on client-side!");
			return;
		}
		...
		if(message.command.equals("server.data.entity.merge")) {
			...
			NBTTagCompound mergeData = message.data.getCompound("entityData");
			...
			theEntity.writeUnlessRemoved(entityData);
			entityData.merge(mergeData);
			theEntity.read(entityData);
		...
		TaleCraft.logger.severe("Received unknown StringNBTCommand from client: "+message.command+" : "+message.data);
	}
